...
function AIDialog({
  ...
  onClose: () => void;
  onApply: (c: string, p: string) => void;
  platforms: string[];
  mode?: "generate" | "enhance";
  initialContent?: string;
}) {
  const [topic, setTopic] = useState(
    mode === "enhance" ? "Refining existing draft..." : "",
  );
  ...
  const generate = async () => {
    ...
    try {
      const endpoint =
        mode === "enhance" ? "/api/ai/enhance-post" : "/api/ai/generate-post";
      const body =
        mode === "enhance"
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 selection:bg-[#2d6a4f44]"
      style={{ background: "rgba(26,31,54,0.45)", backdropFilter: "blur(8px)" }}
    >
      <motion.div
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#f0f3f7] bg-[#fcfdfe]">
          <div className="flex items-center gap-3">
            <div
              className="w-9 h-9 rounded-2xl flex items-center justify-center shadow-lg shadow-[#2d6a4f]/20"
              style={{ background: "linear-gradient(135deg,#2d6a4f,#40916c)" }}
            >
              <Bot className="w-4.5 h-4.5 text-white" />
            </div>
            <div>
              <p className="text-[15px] font-black tracking-tight text-[#1a1f36]">
                {mode === "enhance" ? "Enhance with AI" : "Writer with AI"}
              </p>
              <p className="text-[11px] font-bold text-[#8792a2] uppercase tracking-widest">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full border border-[#e3e8ef] flex items-center justify-center text-[#8792a2] hover:bg-white hover:text-[#2d6a4f] hover:border-[#2d6a4f]/20 transition-all active:scale-[0.95]"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-8 overflow-y-auto custom-scrollbar">
          <div className="space-y-4">
            <label className="block text-[11px] font-black uppercase tracking-[0.25em] text-[#8792a2]">
              Target Platform
            </label>
            <div className="grid grid-cols-4 gap-2">
              {PLATFORMS.slice(0, 4).map((p) => {
                ...
                return (
                  ...
                  >
                    <p.Icon
                      className={`w-4 h-4 ${active ? "text-[#2d6a4f]" : "text-[#8792a2]"}`}
                    />
                    <span
                      className={`text-[10px] font-bold tracking-tight ${active ? "text-[#1a1f36]" : "text-[#8792a2]"}`}
                    >
                      {p.name.split(" ")[0]}
                    </span>
                  </button>
          ...
          <AnimatePresence>
            {result && (
              <motion.div
                ...
                className="rounded-[24px] border border-[#2d6a4f]/15 bg-[#fcfdfe] p-5 shadow-sm relative group"
              >
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[10px] font-black text-[#2d6a4f] uppercase tracking-[0.3em]">
                    AI Output: {primaryPlatform.toUpperCase()}
                  </p>
                </div>
                <p className="text-[14px] text-[#3c4257] font-medium whitespace-pre-wrap leading-relaxed">
                  {result}
                </p>
              </motion.div>
            )}
...
function ImageEditorDialog({
  ...
  const previewUrl = useMemo(() => {
    ...
    if (appliedAiPrompt.trim()) {
      const b64 = btoa(appliedAiPrompt.trim())
        .replace(/\+/g, "-")
        .replace(/\//g, "_")
        .replace(/=+$/, "");
  ...
  return (
    <div
      ...
      style={{ background: "rgba(26,31,54,0.6)", backdropFilter: "blur(12px)" }}
    >
      <motion.div
      ...
      >
        ...
        <div className="flex-1 bg-[#f6f9fc] flex items-center justify-center p-8 relative overflow-hidden">
          ...
          <div className="relative group">
            <img
              src={previewUrl}
              alt="Preview"
              className="max-w-full max-h-full rounded-xl shadow-2xl transition-all duration-500"
        ...
        <div className="w-full md:w-80 border-l border-[#f0f3f7] flex flex-col bg-white">
          <div className="p-6 border-b border-[#f0f3f7] flex items-center justify-between">
            <div>
              <h3 className="text-lg font-black tracking-tight text-[#1a1f36]">
                Edit Image
              </h3>
              <p className="text-[11px] font-bold text-[#8792a2] uppercase tracking-widest">
                ImageKit Engine
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full hover:bg-[#f6f9fc] flex items-center justify-center text-[#8792a2] transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {ASPECT_RATIOS.map((a) => (
                  ...
                  >
                    <a.icon
                      className={`w-4 h-4 ${aspect === a.value ? "text-[#2d6a4f]" : "text-[#8792a2]"}`}
                    />
                    <div>
                      <p
                        className={`text-[12px] font-bold ${aspect === a.value ? "text-[#1a1f36]" : "text-[#8792a2]"}`}
                      >
                        {a.name}
                      </p>
                      {a.desc && (
                        <p className="text-[10px] text-[#c2c8d0] font-medium">
                          {a.desc}
                        </p>
                      )}
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <button
                  onClick={() => setSmartFocus(!smartFocus)}
                ...
                >
                  <span className="text-xs font-bold text-[#3c4257]">
                    Smart Object Focus
                  </span>
                  <div
                    className={`w-8 h-4 rounded-full relative transition-colors ${smartFocus ? "bg-[#2d6a4f]" : "bg-[#e3e8ef]"}`}
                  >
                    <div
                      className={`absolute top-1 w-2 h-2 rounded-full bg-white transition-all ${smartFocus ? "right-1" : "left-1"}`}
                    />
                  </div>
                </button>
                <button
                ...
                >
                  ...
                  <Eraser
                    className={`w-4 h-4 ${bgRemove ? "text-[#2d6a4f]" : "text-[#8792a2]"}`}
                  />
                </button>
              </div>
            </div>
          ...
          <div className="p-6 border-t border-[#f0f3f7] bg-[#fcfdfe] flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 px-6 py-3 rounded-2xl text-[13px] font-extrabold text-[#697386] hover:bg-[#f6f9fc] transition-all active:scale-[0.98]"
            >
              Cancel
            </button>
            <button
...
function PlatformPreview({
  ...
  const noContent = (
    <span className="text-[#c2c8d0] italic">Your post will appear here…</span>
  );

  const safeAccounts = Array.isArray(accounts) ? accounts : [];

  const currentAccount = safeAccounts.find((a) => {
    if (!a?.platform) return false;
    return (
      a.platform.toLowerCase() === platform.id.toLowerCase() ||
      (platform.id === "twitter" && a.platform.toLowerCase() === "twitter / x")
    );
  });
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                <span className="font-bold text-[#0f1419] text-[15px]">
                  {currentAccount?.username || "Your Name"}
                </span>
                <span className="text-[#536471] text-[15px]">
                  @{currentAccount?.username || "YourHandle"}
                </span>
            ...
            {images.length > 0 && !video && (
              <div
                className={`grid gap-0.5 rounded-2xl overflow-hidden border border-[#cfd9de] mt-3 ${images.length === 1 ? "grid-cols-1 aspect-[16/9]" : images.length === 2 ? "grid-cols-2 aspect-[8/4]" : images.length === 3 ? "grid-cols-2 aspect-[8/4]" : "grid-cols-2 aspect-[8/4]"}`}
              >
                {images.map((img, i) => (
                  <div
                    key={i}
                    className={`relative bg-slate-100 ${images.length === 3 && i === 0 ? "row-span-2" : ""}`}
                  >
                    <img
                      src={img.url}
                      alt=""
            ...
            <div className="flex justify-between items-center mt-3 text-[#536471] w-full max-w-[425px]">
              <span className="flex items-center gap-2 group cursor-pointer hover:text-blue-500 transition-colors">
                <MessageCircle className="w-[18px] h-[18px] group-hover:bg-blue-500/10 rounded-full" />
              </span>
              <span className="flex items-center gap-2 group cursor-pointer hover:text-green-500 transition-colors">
                <Repeat2 className="w-[18px] h-[18px] group-hover:bg-green-500/10 rounded-full" />
              </span>
              <span className="flex items-center gap-2 group cursor-pointer hover:text-pink-500 transition-colors">
                <Heart className="w-[18px] h-[18px] group-hover:bg-pink-500/10 rounded-full" />
              </span>
              <span className="flex items-center gap-2 group cursor-pointer hover:text-blue-500 transition-colors">
                <BarChart2 className="w-[18px] h-[18px] group-hover:bg-blue-500/10 rounded-full" />
              </span>
              <span className="flex items-center gap-2 group cursor-pointer hover:text-blue-500 transition-colors">
                <Share className="w-[18px] h-[18px] group-hover:bg-blue-500/10 rounded-full" />
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-3">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-semibold text-sm text-black">
              {currentAccount?.username || "your_handle"}
            </span>
          </div>
        ...
        <div className="p-3">
          <div className="flex justify-between items-center mb-3">
            <div className="flex gap-4">
              ...
              <MessageCircle
                className="w-[24px] h-[24px] text-black hover:text-slate-500 cursor-pointer"
          ...
          <div className="font-semibold text-[14px] leading-none mb-2 text-black">
            0 likes (Preview)
          </div>
      ...
      <div className="bg-white rounded-xl border border-[#e3e8ef] overflow-hidden w-full max-w-lg shadow-sm font-sans mx-auto">
        <div className="p-4 flex gap-3 pb-3">
          ...
          <div className="flex-1">
            <div className="font-semibold text-[14px] text-black/90">
              {currentAccount?.username || "Your Name"}
            </div>
            ...
            <div className="text-[12px] text-black/60 flex items-center gap-1">
              Just now • 🌐
            </div>
          </div>
        ...
        <div className="px-4 py-1.5 border-t border-slate-200 mt-2 flex justify-between">
          {[
            ...
            <button
              key={action.label}
              className="flex items-center gap-1.5 text-[#666666] text-[14px] font-semibold hover:bg-slate-100 px-3 py-2 rounded-md transition-colors"
            >
              <action.icon className="w-[18px] h-[18px] -scale-x-100" />{" "}
              {action.label}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white rounded-xl border border-[#e3e8ef] overflow-hidden w-full max-w-lg shadow-sm font-sans mx-auto pb-2">
        <div className="p-4 pb-2 flex gap-2">
          ...
          <div className="flex-1">
            <div className="font-semibold text-[15px] text-[#050505]">
              {currentAccount?.username || "Your Page"}
            </div>
        ...
        {images.length > 0 && (
          <img
            src={images[0].url}
            alt=""
            className="w-full max-h-[500px] object-cover border-y border-[#e3e8ef]"
          />
        )}
        {video && !images.length && (
          <div className="w-full aspect-video bg-black flex items-center justify-center border-y border-[#e3e8ef]">
            <Film className="w-10 h-10 text-white/50" />
          </div>
        )}
        <div className="px-4 mt-2">
          <div className="flex justify-between border-y border-[#CED0D4] py-1 mt-2">
            {[
              ...
              <button
                key={action.label}
                className="flex flex-1 justify-center items-center gap-1.5 text-[#65676B] font-semibold text-[15px] hover:bg-[#F2F2F2] py-1.5 rounded-md transition-colors"
              >
      ...
      <div className="w-[300px] aspect-[9/16] bg-black rounded-xl overflow-hidden relative mx-auto font-sans shadow-lg">
        ...
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-4 pt-10">
          <div className="font-semibold text-white mb-2">
            @{currentAccount?.username || "your_account"}
          </div>
      ...
      <div className="bg-white rounded-xl overflow-hidden w-full max-w-lg shadow-sm font-sans mx-auto border border-[#e3e8ef]">
        <div className="w-full aspect-video bg-black relative flex items-center justify-center">
          {video || images.length > 0 ? (
            <>
              ...
              <div className="w-16 h-12 bg-red-600 rounded-xl flex items-center justify-center shadow-lg relative z-10">
        ...
        <div className="p-4 flex gap-3">
          ...
          <div className="flex-1 pr-6 relative">
            <div className="font-semibold text-[16px] text-[#0f0f0f] line-clamp-2 leading-tight mb-1">
              {content ? (
                content.split("\n")[0]
              ) : (
                <span className="italic text-slate-400">Video Title</span>
              )}
            </div>
    ...
    return (
      <div className="bg-white rounded-xl border border-[#e3e8ef] overflow-hidden w-full max-w-xs shadow-sm font-sans mx-auto p-4 flex flex-col">
        {images.length > 0 ? (
          ...
          <div className="w-full aspect-[2/3] bg-slate-100 rounded-2xl flex items-center justify-center mb-4 text-slate-400">
            Image Required
          </div>
        )}
        <div className="flex items-center justify-between mb-2">
          <MoreHorizontal className="w-6 h-6 text-[#111]" />
          <button className="bg-[#E60023] text-white font-bold text-[15px] px-4 py-3 rounded-full leading-none hover:bg-red-700 transition-colors">
            Save
          </button>
        </div>
        {content && (
    ...
    <div className="rounded-xl border border-[#e3e8ef] bg-white p-4 shadow-sm mx-auto w-full max-w-lg">
      ...
      {images.length > 0 && (
        <img
          src={images[0].url}
          alt=""
          className="mt-3 rounded-xl w-full object-cover max-h-44"
        />
      )}
    </div>
  );
    ...
    <div className="bg-white rounded-xl border border-[#e3e8ef] shadow-[0_1px_3px_rgba(60,66,87,0.05)] overflow-hidden">
      {title && (
        <div className="px-5 py-4 border-b border-[#f0f3f7] bg-[#f6f9fc]">
          ...
          {subtitle && (
            <p className="text-xs text-[#8792a2] mt-0.5">{subtitle}</p>
          )}
        </div>
      )}
...
export default function ComposePage() {
  ...
  // Make sure preview platform is connected
  useEffect(() => {
    ...
    if (
      ...
      !isPlatformConnected(previewPlatform)
    ) {
      setPreviewPlatform(connectedPlatforms[0].id);
    }
    // Also update selected platforms to only connected ones
    setSelected((prev) => prev.filter((id) => isPlatformConnected(id)));
  }, [accounts]);
  ...
  const isPlatformConnected = (platformId: string) => {
    return accounts.some(
      (a) =>
        ...
        (platformId === "twitter" &&
          a.platform.toLowerCase() === "twitter / x"),
    );
  };

  // Helper to check if any media is a GIF
  const hasGifMedia = () => {
    return media.some(
      (m) =>
        m.name?.toLowerCase().endsWith(".gif") ||
  ...
  const onEmoji = (e: EmojiClickData) => {
    ...
    setTimeout(() => {
      ta.focus();
      ta.selectionStart = ta.selectionEnd = s + e.emoji.length;
    }, 0);
  };
  ...
  const hashtags = async () => {
    ...
    try {
      ...
      if (d.hashtags) {
        setContent((p) => p + "\n\n" + d.hashtags);
        toast.success("Hashtags added! #️⃣");
      }
    } catch {
      toast.error("Failed");
    } finally {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : "Upload failed - using proxy",
      );
    } finally {
      setUploading(false);
    }
  ...
  const fetchBest = async () => {
    setLoadingBest(true);
    try {
      const r = await fetch("/api/ai/best-time", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (d.datetime) {
        setBestTime(d);
        const [dd, tt] = d.datetime.split("T");
        setDate(dd);
        setTime(tt?.slice(0, 5) || "10:00");
        setMode("scheduled");
      }
    } catch {
      toast.error("Could not suggest best time");
    } finally {
  ...
  const submit = async () => {
    ...
    if (selected.includes("instagram") && hasGifMedia()) {
      toast.error(
        "Instagram does not support GIFs - please use JPG or PNG instead",
      );
      return;
    }
    setSubmitting(true);
    try {
      const scheduledAt =
        mode === "scheduled" && date ? `${date}T${time}:00` : null;
  ...
  const warnings = (id: string) => {
    ...
    if (!p) return [];
    const w: string[] = [];
    if (p.charLimit && content.length > p.charLimit)
      w.push(`Over character limit (${content.length}/${p.charLimit})`);
    if (p.requiresVideo && media.every((m) => m.type !== "video"))
      w.push("Video content required");
    if (p.requiresMedia && media.length === 0)
      w.push("Media strongly recommended");
    if (id === "instagram" && hasGifMedia())
      w.push("Instagram does not support GIFs - use JPG or PNG instead");
    return w;
  };
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-[#8792a2] mt-1">
            Compose and schedule across all your connected platforms.
          </p>
        </div>
        {/* AI Generate CTA */}
        <button
        ...
        <div className="flex items-center">
          {STEPS.map((label, i) => (
            ...
            >
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
                    i < step
                      ? "bg-[#09825d] border-[#09825d] text-white"
                      : i === step
                        ? "bg-[#2d6a4f] border-[#2d6a4f] text-white shadow-[0_0_0_3px_rgba(99,91,255,0.15)]"
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={`flex-1 h-px mx-3 mb-5 transition-all ${i < step ? "bg-[#09825d]" : "bg-[#e3e8ef]"}`}
                />
              )}
            </div>
          ))}
        ...
        >
            ...
            <div className="bg-white rounded-xl border border-[#e3e8ef] shadow-[0_1px_3px_rgba(60,66,87,0.05)] overflow-hidden">
              ...
              <div className="flex items-center gap-1 px-4 py-2.5 border-b border-[#f0f3f7]">
                <div className="relative">
                  ...
                  {showEmoji && (
                    <div className="absolute top-9 left-0 z-30 shadow-[0_8px_30px_rgba(60,66,87,0.12)] rounded-xl overflow-hidden">
                      <EmojiPicker
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-[#f0f3f7] bg-[#f6f9fc]">
                <div className="flex flex-wrap gap-1.5">
                  {PLATFORMS.filter(
                    (p) => p.charLimit && p.charLimit <= 3000,
                  ).map((p) => (
                    <span
                      key={p.id}
                ...
                <span className="text-[10px] text-[#c2c8d0] font-medium">
          ...
          {step === 1 && (
            <div className="space-y-4">
              <Card
                title="Add media"
                subtitle="Attach images or videos to your post"
              >
                {/* Tab bar */}
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center gap-3">
                      {uploading ? (
                        ...
                        <div className="w-12 h-12 rounded-xl bg-[#f6f9fc] border border-[#e3e8ef] flex items-center justify-center">
                          <Upload className="w-5 h-5 text-[#8792a2]" />
                        </div>
                      )}
                      <div>
                        <p className="text-sm font-semibold text-[#1a1f36]">
                        ...
                        <p className="text-xs text-[#8792a2] mt-1">
                          JPG, PNG, GIF, MP4, MOV · Max 100 MB
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {mediaTab === "library" &&
                  (loadingLib ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      <FileImage className="w-9 h-9 text-[#c2c8d0] mb-3" />
                      <p className="text-sm font-medium text-[#697386]">
                        No uploads yet
                      </p>
                      <p className="text-xs text-[#c2c8d0] mt-1">
                        Upload files to build your library
                      </p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-2.5">
                      {library.map((a) => {
                        const sel = media.some(
                          (m) => m.fileId === a.imageKitFileId,
                        );
                        return (
                          <button
                            key={a.id}
                            onClick={() =>
                              sel
                                ...
                                : setMedia((p) => [
                                    ...
                                    {
                                      url: a.url,
                                      fileId: a.imageKitFileId,
                                      type: a.type as "image" | "video",
                                      name: a.imageKitFileId,
                                    },
                                  ])
                            }
                          ...
                          >
                            {a.type === "video" ? (
                              <div className="w-full h-full bg-[#1a1f36] flex items-center justify-center">
                ...
                <Card title={`Selected files (${media.length})`}>
                  <div className="flex flex-wrap gap-3">
                    {media.map((m, i) => (
                      <div
                        key={i}
                        className="relative w-24 h-24 rounded-xl overflow-hidden border-2 border-[#e3e8ef] group shadow-[0_4px_12px_rgba(60,66,87,0.08)] transition-all hover:border-[#2d6a4f]/30"
                      >
                        {m.type === "video" ? (
                          <div className="w-full h-full bg-[#1a1f36] flex items-center justify-center">
                  ...
                  {[
                    {
                      ...
                      color: "#E1306C",
                      name: "Instagram",
                      note: "Square 1:1 or portrait 4:5",
                    },
                    {
                      Icon: FaXTwitter,
                      color: "#000",
                      name: "X (Twitter)",
                      note: "Max 4 images or 1 video",
                    },
                    {
                      Icon: SiTiktok,
                      color: "#010101",
                      name: "TikTok",
                      note: "Vertical 9:16 video required",
                    },
                    {
                      Icon: FaYoutube,
                      color: "#FF0000",
                      name: "YouTube",
                      note: "Horizontal 16:9 video preferred",
                    },
                    ...
                    >
                      <Icon
                        ...
                        style={{ color }}
                      />
                      <span className="text-xs font-semibold text-[#3c4257] w-24 shrink-0">
                        {name}
                      </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
              ...
              <div className="lg:col-span-2 space-y-4">
                <Card
                  ...
                  subtitle="Toggle the accounts to publish to"
                >
                  <div className="space-y-2">
                    {PLATFORMS.filter((p) => isPlatformConnected(p.id)).map(
                      (p) => {
                        ...
                        return (
                          <div
                            key={p.id}
                            className={`rounded-lg border-2 overflow-hidden transition-all ${on ? "border-[#2d6a4f]" : "border-[#e3e8ef] hover:border-[#2d6a4f]/30"}`}
                          >
                            <div
                              className={`flex items-center gap-3 px-3.5 py-3 cursor-pointer ${on ? "bg-[#2d6a4f]/4" : "bg-white"}`}
                              onClick={() =>
                                setSelected((p2) =>
                                  on
                                    ? p2.filter((x) => x !== p.id)
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {p.charLimit && (
                                  <p className="text-[10px] text-[#8792a2]">
                                    {p.charLimit.toLocaleString()} char limit
                                  </p>
                                )}
                              </div>
                              <div
                                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${on ? "bg-[#2d6a4f] border-[#2d6a4f]" : "border-[#d1d5db]"}`}
                              >
                            ...
                            {on &&
                              ws.map((w) => (
                                <div
                                  key={w}
                                  className="flex items-center gap-2 px-3.5 py-2 bg-amber-50 border-t border-amber-100"
                                >
                    ...
                    {PLATFORMS.filter((p) => isPlatformConnected(p.id))
                      .length === 0 && (
                      <div className="text-center py-8 text-[#697386]">
                        <p className="text-sm">No connected platforms yet!</p>
                        <p className="text-xs mt-1">
                          Connect platforms in the Connections page
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-5">
                    {selected.length === 0 ? (
                      ...
                      <div className="space-y-4">
                            ...
                            return (
                              <button
                                ...
                                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                                  previewPlatform === pid
                                    ? "border-[#2d6a4f] text-[#2d6a4f] bg-[#2d6a4f]/8"
                                    : "border-[#e3e8ef] text-[#697386] hover:border-[#2d6a4f]/30 hover:text-[#2d6a4f]"
                                }`}
                              >
                        ...
                        {/* Preview card */}
                        {PLATFORMS.filter((p) => p.id === previewPlatform).map(
                          (p) => (
                            <PlatformPreview
                              key={p.id}
                              platform={p}
              ...
              <div className="bg-white rounded-xl border border-[#e3e8ef] shadow-[0_1px_3px_rgba(60,66,87,0.05)] overflow-hidden">
                ...
                <div className="p-5 grid grid-cols-2 gap-3">
                  {(
                    [
                      {
                        ...
                        icon: Send,
                        label: "Publish Now",
                        desc: "Post immediately to all selected platforms",
                      },
                      {
                        v: "scheduled",
                        icon: Calendar,
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${mode === v ? "bg-[#2d6a4f] text-white" : "bg-[#f6f9fc] text-[#8792a2]"}`}
                      >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#8792a2] mt-0.5 leading-relaxed">
                          {desc}
                        </p>
                      </div>
                    </button>
                ...
                >
                  ...
                  <div className="p-5 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-[#3c4257] mb-1.5">
                          Date
                        </label>
                        <input
                          type="date"
                    ...
                    <div className="flex items-center gap-3 pt-1 border-t border-[#f0f3f7]">
                      ...
                      >
                        {loadingBest ? (
                          ...
                          <ZapIcon className="w-3.5 h-3.5 text-[#2d6a4f]" />
                        )}
                        AI Best Time
                      </button>
                      {bestTime && (
                        <p className="text-xs text-[#8792a2] leading-relaxed flex-1">
                          {bestTime.reason}
                        </p>
                      )}
                    </div>
                  ...
                  {[
                    {
                      ...
                      value: content ? (
                        ...
                        <span className="text-[#c2c8d0]">None</span>
                      ),
                    },
                    {
                      label: "Media",
                      value: `${media.length} file${media.length !== 1 ? "s" : ""}`,
                    },
                    {
                      label: "Platforms",
                      value:
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {selected.map((pid) => {
                              ...
                              return (
                                <span
                                  key={pid}
                                  className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border border-[#e3e8ef]"
                                  style={{ color: p.color }}
                                >
                                  <p.Icon className="w-2.5 h-2.5" />
                                  {p.name}
                    ...
                    },
                    {
                      ...
                      value:
                        mode === "now" ? (
                          ...
                          <span className="text-[#c2c8d0]">Not set</span>
                        ),
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                      className="flex items-center gap-4 px-5 py-3.5 hover:bg-[#f6f9fc] transition-colors"
                    >
                      <span className="text-xs font-semibold text-[#8792a2] w-20 shrink-0">
                        {label}
                      </span>
                      <span className="text-sm text-[#3c4257] flex-1 min-w-0">
                        {value}
                      </span>
                    </div>
                  ))}
